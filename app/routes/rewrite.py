...
async def export_rewrite_docx(
    request: ExportDocxRequest,
    authorization: str = Header(default=""),
):
    """Generate a .docx file with the rewritten text.

    Free users get a watermarked document. Pro/Premium get clean export.
    ...
    # Resolve user tier for watermark decision
    ...
    is_paid = tier in (UserTier.PRO, UserTier.PREMIUM)
    ...
    # Watermark for free/anonymous users
    if not is_paid:
        wm = doc.add_paragraph()
        wm_run = wm.add_run("⚠ FREE PLAN — Upgrade to Pro for clean exports without watermark")
        wm_run.font.size = Pt(10)
        wm_run.font.color.rgb = RGBColor(0xEF, 0x44, 0x44)
        wm_run.bold = True
    ...
    # Add footer note
    doc.add_paragraph("")
    footer_text = "Generated by PlagiarismGuard — plagiarismguard.com"
    if not is_paid:
        footer_text += " | FREE PLAN — Upgrade for watermark-free exports"

...
const STAGES: Stage[] = [
  { label: "Input" },
  { label: "Scan", sub: "Stage 01" },
  { label: "Verify", sub: "Stage 02" },
  { label: "Attribute", sub: "Stage 03" },
  { label: "Report" },
];
const CYCLE_MS = 6000;
...
export default function PipelineFlow() {
  ...
  return (
    <div className="mb-16">
      <div className="flex items-end justify-center gap-1 sm:gap-2 text-[11px] uppercase tracking-wider overflow-x-auto pb-2">
        {STAGES.map((stage, i) => {
          ...
          return (
            <div key={stage.label} className="flex items-end gap-1 sm:gap-2">
              <div className="flex flex-col items-center gap-1.5">
                <span
                  ...
                  style={{
                    ...
                    boxShadow: lit
                      ? isReport
                        ? `0 0 0 4px rgb(var(--ok-rgb, 34 197 94) / ${0.08 + glow})`
                        : `0 0 0 4px rgb(var(--txt-rgb) / ${0.08 + glow})`
                      : "none",
                  }}
                >
                  {intensity > 0.7 && (
                    <span
                      ...
                      style={{
                        background: isReport
                          ? "rgb(var(--ok-rgb, 34 197 94) / 0.35)"
                          : "rgb(var(--txt-rgb) / 0.35)",
                ...
                {stage.sub ? (
                  <span className="text-[9px] tracking-[0.15em] text-muted/60 font-normal normal-case">
                    {stage.sub}
                  </span>
                ) : (
                  <span className="text-[9px] tracking-[0.15em] text-muted/40 font-normal normal-case">
                    {stage.label === "Input" ? "your doc" : "audit trail"}
                  </span>
                )}
              </div>
              {i < STAGES.length - 1 && (
...
function Connector({ progress, index }: { progress: number; index: number }) {
  ...
  return (
    <div className="relative h-px w-6 sm:w-10 shrink-0 overflow-hidden rounded-full bg-border/40">
      <span
      ...
      {crossing && (
        <span
          ...
          style={{
            ...
            background:
              "radial-gradient(ellipse at center, rgb(var(--txt-rgb) / 0.9), rgb(var(--txt-rgb) / 0) 70%)",
            filter: "blur(1px)",
          }}
        />
      )}
